...
When a user uploads a PDF, analyze the extracted text and provide documentary-specific insights.
...
const SAMPLE_SESSIONS = [
  ...
  { id: 2, title: "Women in Tech India",   phase: "Narrative", time: "2 days ago", color: "#db2777" },
];
    ...
    <div style={{ display: "flex", gap: 5, alignItems: "center", padding: "2px 0" }}>
      {[0, 1, 2].map(i => (
        <div key={i} style={{
          ...
          animation: `typeBounce 1.4s ${i * 0.2}s infinite ease-in-out`,
        }} />
      ))}
      <style>{`@keyframes typeBounce{0%,80%,100%{transform:scale(0.8);opacity:.5}40%{transform:scale(1.2);opacity:1}}`}</style>
    </div>
  );
}
...
function Avatar({ initials, size = 36, gradient = "linear-gradient(135deg,#6366f1,#7c3aed)" }) {
  return (
    <div style={{
      ...
      justifyContent: "center", fontSize: size * 0.35, fontWeight: 800,
      color: "#fff", flexShrink: 0, boxShadow: "0 4px 14px rgba(99,102,241,.3)",
    }}>
      {initials}
    </div>
  );
...
function Message({ msg, userInitials }) {
  ...
  return (
    <div style={{
      ...
      alignItems: "flex-end", gap: 12, marginBottom: 24,
      animation: "msgIn .4s cubic-bezier(.34,1.56,.64,1)",
    }}>
      <style>{`@keyframes msgIn{from{opacity:0;transform:translateY(16px) scale(.95)}to{opacity:1;transform:translateY(0) scale(1)}}`}</style>
      ...
      <div style={{
        maxWidth: "73%", display: "flex", flexDirection: "column",
        ...
        {msg.isPdf && (
          <div style={{
            ...
            border: "1px solid #ddd6fe", display: "flex", alignItems: "center", gap: 8,
          }}>
        ...
        <div style={{
          ...
          background: isUser
            ? "linear-gradient(135deg,#6366f1,#7c3aed,#db2777)"
            : "white",
          color: isUser ? "#fff" : "#1e293b",
          fontSize: 14.5, lineHeight: 1.75,
          boxShadow: isUser
            ? "0 8px 30px rgba(99,102,241,.3)"
...
export default function App() {
  ...
  // ── PDF upload (FIXED: actually extracts text) ────────────────────────────
  const handlePdf = async (e) => {
    const file = e.target.files[0];
    if (!file) return;
    if (file.type !== "application/pdf") { alert("PDF only!"); return; }
    if (file.size > 5e6)                 { alert("Max 5 MB!"); return; }

    // Show user bubble immediately
    setMessages(p => [...p, {
      role: "user", content: `Analyse this PDF: "${file.name}" for documentary insights.`,
      isPdf: true, fileName: file.name,
    }]);
    ...
    try {
      ...
      setMessages(p => [...p, { role: "assistant", content: `❌ Could not read PDF: ${err.message}` }]);
      setPdfUploading(false); setLoading(false);
      e.target.value = "";
      return;
    }
    ...
    const userMsg = extractedText.length === 0
      ? `[Phase: ${PHASES[phase].label}] User uploaded PDF "${file.name}" but it contained no extractable text (it may be a scanned/image PDF). Acknowledge this and suggest how they might still use the tool.`
      : `[Phase: ${PHASES[phase].label}] User uploaded PDF "${file.name}". Here is the extracted text:\n\n${truncated}\n\nPlease analyse this content and provide documentary-specific insights.`;

    const nh = [...history, { role: "user", content: userMsg }];
    ...
    try {
      const res = await fetch("https://api.groq.com/openai/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          max_tokens: 1000,
          messages: [{ role: "system", content: SYSTEM_PROMPT }, ...nh],
        }),
      });
      ...
      setSessions(p => [{
        id: Date.now(), title: file.name.slice(0, 26),
        phase: PHASES[phase].label, time: "Just now", color: PHASES[phase].color,
      }, ...p.slice(0, 9)]);
    } catch (err) {
      setMessages(p => [...p, { role: "assistant", content: `❌ API error: ${err.message}` }]);
    }
  ...
  const send = async (text, isVoice = false) => {
    ...
    try {
      ...
      setSessions(p => [{
        id: Date.now(),
        title: msg.slice(0, 28) + (msg.length > 28 ? "…" : ""),
        phase: PHASES[phase].label, time: "Just now", color: PHASES[phase].color,
      }, ...p.slice(0, 9)]);
    } catch (err) {
  ...
  const switchPhase = idx => {
    ...
    setMessages(p => [...p, {
      role: "assistant",
      content: `${PHASES[idx].icon} Switched to **${PHASES[idx].label}** phase!\n\n${PHASES[idx].desc} — what would you like to work on?`,
    }]);
  };

  const newChat = () => {
    setMessages([{ role: "assistant", content: "New session! 🎬\n\n🎤 Speak  •  📄 Upload PDF  •  ⌨️ Type\n\nWhat's your documentary topic?" }]);
    setHistory([]); setActiveSession(null);
  };
  ...
  const handleLogin = () => {
    if (loginForm.name && loginForm.email) {
      setUser({
        ...
        initials: loginForm.name.split(" ").map(w => w[0]).join("").toUpperCase().slice(0, 2),
      });
    ...
    <div style={{
      ...
      <style>{`
        *{box-sizing:border-box}
        ...
        @keyframes pulseGreen{0%,100%{box-shadow:0 0 0 0 rgba(16,185,129,.4)}70%{box-shadow:0 0 0 8px rgba(16,185,129,0)}}
        ...
        button{font-family:inherit;cursor:pointer;border:none;transition:all .2s cubic-bezier(.34,1.56,.64,1)}
        .hoverlift:hover{transform:translateY(-2px)!important}
        .hoverscale:hover{transform:scale(1.06)!important}
        .sessionrow:hover{background:linear-gradient(135deg,rgba(99,102,241,.06),rgba(124,58,237,.06))!important;transform:translateX(4px)!important}
      `}</style>

      {/* decorative blobs */}
      <div style={{ position:"fixed", top:-100, right:-100, width:400, height:400, borderRadius:"50%", background:"radial-gradient(circle,rgba(99,102,241,.08),transparent 70%)", pointerEvents:"none", zIndex:0 }} />
      <div style={{ position:"fixed", bottom:-80, left:200, width:300, height:300, borderRadius:"50%", background:"radial-gradient(circle,rgba(219,39,119,.06),transparent 70%)", pointerEvents:"none", zIndex:0 }} />

      {/* ══════════ SIDEBAR ══════════ */}
      {sidebarOpen && (
        <div style={{
          width:272, background:"white", borderRight:"1px solid #e8eaf6",
          display:"flex", flexDirection:"column", flexShrink:0,
          animation:"slideIn .28s ease", boxShadow:"4px 0 32px rgba(99,102,241,.07)",
          ...
          <div style={{ padding:"22px 18px 16px" }}>
            <div style={{ display:"flex", alignItems:"center", gap:11, marginBottom:16 }}>
              <div style={{
                ...
                background:"linear-gradient(135deg,#6366f1,#7c3aed,#db2777)",
                backgroundSize:"200% 200%",
                animation:"gradientShift 4s ease infinite, floatAnim 3s ease infinite",
                display:"flex", alignItems:"center", justifyContent:"center",
                fontSize:20, boxShadow:"0 6px 20px rgba(99,102,241,.35)",
              }}>🎬</div>
              <div>
                <div style={{ fontSize:15, fontWeight:800, color:"#1e293b", letterSpacing:"-.4px" }}>DocCreator</div>
          ...
          <div style={{ padding:"0 16px 12px" }}>
            <div style={{ fontSize:10, color:"#94a3b8", fontWeight:700, letterSpacing:".1em", textTransform:"uppercase", marginBottom:8 }}>Workflow</div>
            <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
              {PHASES.map(p => (
                <button key={p.id} onClick={() => switchPhase(p.id)} style={{
                  padding:"9px 14px", borderRadius:12,
                  border:`1.5px solid ${phase === p.id ? p.border : "transparent"}`,
                  background: phase === p.id ? p.light : "transparent",
                  color: phase === p.id ? p.color : "#64748b",
                  fontSize:12, fontWeight: phase === p.id ? 700 : 500,
                  display:"flex", alignItems:"center", gap:8, textAlign:"left",
                  boxShadow: phase === p.id ? `0 2px 12px ${p.color}20` : "none",
                  transition:"all .18s",
                }}>
                  <span style={{ fontSize:15 }}>{p.icon}</span>
                  <div>
                    <div style={{ fontSize:12, fontWeight: phase === p.id ? 700 : 500 }}>{p.label}</div>
                    <div style={{ fontSize:10, color: phase === p.id ? p.color : "#94a3b8", opacity:.8 }}>{p.desc}</div>
                  </div>
          ...
          <div style={{ flex:1, overflowY:"auto", padding:"0 10px" }}>
            <div style={{ fontSize:10, color:"#94a3b8", fontWeight:700, letterSpacing:".1em", textTransform:"uppercase", padding:"4px 8px 8px" }}>Recent Chats</div>
            {sessions.map(s => (
              <div key={s.id} className="sessionrow" onClick={() => setActiveSession(s.id)} style={{
                padding:"10px 12px", borderRadius:12, marginBottom:3, cursor:"pointer",
                background: activeSession === s.id ? `${s.color}10` : "transparent",
                border:`1px solid ${activeSession === s.id ? s.color+"30" : "transparent"}`,
                transition:"all .2s",
              }}>
          ...
          <div style={{ padding:"12px 14px 16px", borderTop:"1px solid #f1f5f9" }}>
            <div onClick={() => setShowUserMenu(!showUserMenu)} style={{
              ...
              background: showUserMenu ? "#f8faff" : "transparent",
              border:`1px solid ${showUserMenu ? "#e0e7ff" : "transparent"}`,
              transition:"all .18s",
            }}>
            ...
            {showUserMenu && (
              <div style={{ marginTop:8, borderRadius:14, background:"white", border:"1px solid #e8eaf6", boxShadow:"0 8px 32px rgba(99,102,241,.12)", overflow:"hidden" }}>
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", position:"relative", zIndex:1 }}>
        ...
        {/* topbar */}
        <div style={{
          padding:"0 24px", height:64, borderBottom:"1px solid #eef2ff",
          display:"flex", alignItems:"center", gap:14,
          background:"rgba(255,255,255,.9)", backdropFilter:"blur(20px)",
          flexShrink:0, boxShadow:"0 1px 24px rgba(99,102,241,.06)",
        }}>
          {!sidebarOpen && (
          ...
          <div>
            <div style={{ fontSize:16, fontWeight:800, color:"#1e293b", letterSpacing:"-.4px" }}>Documentary Creator</div>
          ...
          <div style={{ display:"flex", gap:6, marginLeft:"auto", marginRight:"auto" }}>
            {PHASES.map(p => (
              <button key={p.id} className="hoverlift" onClick={() => switchPhase(p.id)} style={{
                padding:"7px 16px", borderRadius:99,
                border:`1.5px solid ${phase === p.id ? p.color : "#e8eaf6"}`,
                background: phase === p.id ? `linear-gradient(135deg,${p.light},white)` : "white",
                color: phase === p.id ? p.color : "#94a3b8",
                fontSize:12, fontWeight: phase === p.id ? 800 : 500,
                display:"flex", alignItems:"center", gap:5,
                boxShadow: phase === p.id ? `0 4px 16px ${p.color}25` : "0 1px 4px rgba(0,0,0,.04)",
              }}>
          ...
          <div style={{ background:"linear-gradient(135deg,#fef2f2,#fff1f2)", borderBottom:"1px solid #fecaca", padding:"10px 28px", display:"flex", alignItems:"center", gap:14, flexShrink:0 }}>
            ...
            <div style={{ display:"flex", gap:2, alignItems:"center" }}>
              {[3,5,7,9,7,5,7,9,7,5,3,5,7,5,3].map((h,i) => (
                <div key={i} style={{ width:3, height:h*2.5, background:"linear-gradient(to top,#ef4444,#fb7185)", borderRadius:3, animation:`wave .6s ${i*.06}s infinite` }} />
              ))}
            </div>
        ...
        <div style={{ height:3, background:"#f1f5f9", flexShrink:0, display:"flex" }}>
          {PHASES.map((p,i) => (
            <div key={i} style={{
              flex:1,
              background: i <= phase ? `linear-gradient(90deg,${p.color},${PHASES[Math.min(i+1,3)].color})` : "transparent",
        ...
        <div style={{ padding:"10px 28px", background:"white", borderBottom:"1px solid #f1f5f9", display:"flex", alignItems:"center", gap:10, flexShrink:0 }}>
          ...
          <div style={{ marginLeft:"auto", display:"flex", gap:5, alignItems:"center" }}>
            ...
            {PHASES.map((p,i) => (
              <div key={i} className="hoverlift" onClick={() => switchPhase(i)} style={{
                width: i === phase ? 28 : 10, height:8, borderRadius:99,
                background: i <= phase ? p.color : "#e8eaf6",
                transition:"all .3s", cursor:"pointer",
                boxShadow: i === phase ? `0 2px 8px ${p.color}50` : "none",
              }} />
            ))}
          </div>
        </div>
          ...
          <div style={{ maxWidth:820, margin:"0 auto" }}>
            ...
            {(loading || pdfUploading) && (
              <div style={{ display:"flex", alignItems:"flex-end", gap:12, marginBottom:20 }}>
                ...
                <div style={{ padding:"16px 20px", borderRadius:"22px 22px 22px 6px", background:"white", border:"1px solid #f1f5f9", boxShadow:"0 4px 24px rgba(0,0,0,.06)" }}>
                  {pdfUploading
                    ? <span style={{ fontSize:13, color:"#7c3aed", fontWeight:600 }}>📄 Extracting PDF text…</span>
                    : <TypingDots />
                  }
                </div>
              </div>
        ...
        <div style={{ padding:"10px 32px 8px", background:"white", borderTop:"1px solid #f1f5f9" }}>
          <div style={{ maxWidth:820, margin:"0 auto", display:"flex", gap:7, overflowX:"auto", scrollbarWidth:"none" }}>
            {QUICK_PROMPTS.map(q => (
              <button key={q.text} className="hoverlift" onClick={() => send(q.text)} style={{
                whiteSpace:"nowrap", padding:"7px 16px", borderRadius:99,
                border:`1.5px solid ${q.color}25`, background:`${q.color}08`,
                color:q.color, fontSize:12, fontWeight:600,
                display:"flex", alignItems:"center", gap:5,
                boxShadow:`0 2px 8px ${q.color}10`,
              }}>
        ...
        {/* input area */}
        <div style={{ padding:"10px 32px 20px", background:"white", boxShadow:"0 -4px 32px rgba(99,102,241,.06)" }}>
          <div style={{ maxWidth:820, margin:"0 auto" }}>

            {/* upload row */}
            <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:10 }}>
              <button onClick={() => fileRef.current?.click()} className="hoverlift" style={{
                display:"flex", alignItems:"center", gap:6, padding:"6px 14px",
                borderRadius:10, border:"1.5px solid #ddd6fe",
                background:"linear-gradient(135deg,#faf5ff,#f5f3ff)",
                color:"#7c3aed", fontSize:12, fontWeight:700,
                boxShadow:"0 2px 10px rgba(124,58,237,.1)",
              }}>
                📄 Upload PDF Script
              </button>
            ...
            <div style={{ display:"flex", gap:10, alignItems:"flex-end" }}>
              <div style={{
                flex:1,
                background: isRecording ? "#fff5f5" : "#f8faff",
                border:`2px solid ${isRecording ? "#fca5a5" : "#e0e7ff"}`,
                borderRadius:20, padding:"14px 20px",
                boxShadow: isRecording
                  ? "0 0 0 4px rgba(239,68,68,.08), 0 4px 24px rgba(0,0,0,.06)"
                ...
                <textarea
                  ...
                  style={{
                    ...
                    fontSize:14.5, lineHeight:1.6, maxHeight:130,
                  }}
                />
              </div>

              {/* mic */}
              <button onClick={toggleVoice} className="hoverscale" style={{
                width:52, height:52, borderRadius:16,
                background: isRecording
                  ? "linear-gradient(135deg,#ef4444,#dc2626)"
                  : "linear-gradient(135deg,#fef2f2,#fee2e2)",
                border: isRecording ? "none" : "1.5px solid #fecaca",
                display:"flex", alignItems:"center", justifyContent:"center",
                fontSize:22, flexShrink:0,
                boxShadow: isRecording ? "0 0 28px rgba(239,68,68,.55)" : "0 3px 14px rgba(239,68,68,.15)",
              ...
              {/* send */}
              <button onClick={() => send()} disabled={!input.trim() || loading} className="hoverscale" style={{
                width:52, height:52, borderRadius:16,
                background: input.trim() && !loading
                  ? "linear-gradient(135deg,#6366f1,#7c3aed,#db2777)"
                  : "#f1f5f9",
                backgroundSize:"200% 200%",
                animation: input.trim() && !loading ? "gradientShift 3s ease infinite" : "none",
                border:"none", display:"flex", alignItems:"center", justifyContent:"center",
                fontSize:22, flexShrink:0,
                boxShadow: input.trim() && !loading ? "0 6px 28px rgba(99,102,241,.4)" : "none",
      ...
      {showLogin && (
        <div style={{
          ...
          <div style={{
            width:400, background:"white", borderRadius:28, padding:36,
            boxShadow:"0 32px 80px rgba(99,102,241,.2)",
            ...
            <div style={{ textAlign:"center", marginBottom:28 }}>
              ...
              <div style={{ fontSize:22, fontWeight:800, color:"#1e293b", letterSpacing:"-.4px" }}>Your Profile</div>
            ...
            {[{ label:"Full Name", key:"name", ph:user.name },{ label:"Email Address", key:"email", ph:user.email }].map(f => (
              <div key={f.key} style={{ marginBottom:16 }}>
                <div style={{ fontSize:12, color:"#6366f1", marginBottom:7, fontWeight:700, letterSpacing:".04em" }}>{f.label}</div>
                <input
            ...
            <div style={{ display:"flex", gap:10, marginTop:24 }}>
              <button onClick={() => setShowLogin(false)} style={{ flex:1, padding:13, borderRadius:14, background:"#f8faff", color:"#64748b", fontSize:14, fontWeight:600, border:"1.5px solid #e0e7ff" }}>Cancel</button>
              <button onClick={handleLogin} style={{ flex:1, padding:13, borderRadius:14, background:"linear-gradient(135deg,#6366f1,#7c3aed)", color:"#fff", fontSize:14, fontWeight:800, boxShadow:"0 6px 24px rgba(99,102,241,.35)", letterSpacing:".02em" }}>Save ✓</button>
            </div>
          </div>
        </div>
      )}
